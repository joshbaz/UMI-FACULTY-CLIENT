...
import { createExternalPersonService, addNewPanelistService, createChairpersonService, scheduleProposalDefenseService, createPanelistFromStaffService, createReviewerFromStaffService } from "../../store/tanstackStore/services/api";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const AddPersonDialog = ({ 
  ...
  const createReviewerFromStaffMutation = useMutation({
    mutationFn: createReviewerFromStaffService,
    onSuccess: (data) => {
      setConversionLoading(false);
      setShowConversionDialog(false);
      setSelectedStaffMember(null);
      ...
      queryClient.resetQueries({ queryKey: ['staffMembers'] });
      queryClient.invalidateQueries({ queryKey: ['staffMembers'] });
      toast.success('Staff member converted to reviewer successfully');
    },
    onError: (err) => {
      setConversionLoading(false);
      setError(err.response?.data?.message || "Failed to convert staff member to reviewer");
    }
  });
  ...
  const getAvailablePeople = () => {
    if (type === 'panelist') {
      ...
      const allStaffMembers = staffMembers.map(staff => ({
        ...
        department: staff.department,
        isStaffMember: true,
        isAlreadyReviewer: !!staff.reviewerId
      }));
  ...
  const handleConversionConfirm = () => {
    ...
    if (type === 'panelist') {
      ...
      createReviewerFromStaffMutation.mutate(selectedStaffMember.id);
    }
  };
                    ...
                    {filteredPeople.map((person) => (
                      <div
                        ...
                        onClick={() => {
                          if (person.isStaffMember && !person.isAlreadyPanelist && type === 'panelist') {
                          ...
                          } else if (person.isStaffMember && !person.isAlreadyReviewer && type === 'reviewer') {
                            handleStaffMemberSelect(person);
                            ...
                            // Use the reviewerId for staff members who are already reviewers
                            onSelect(person.reviewerId);
                          } else {
                            onSelect(person.id);
                          }
                        }}
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                      This staff member will be converted to a {type === 'panelist' ? 'panelist' : 'reviewer'} role. The conversion will:
                    </p>
                    <ul className="text-sm text-yellow-700 mt-2 space-y-1">
                      <li>• Create a new {type === 'panelist' ? 'panelist' : 'reviewer'} record</li>
                      <li>• Link the staff member to the {type === 'panelist' ? 'panelist' : 'reviewer'} role</li>
                      <li>• Allow the staff member to serve as a {type === 'panelist' ? 'panelist' : 'reviewer'}</li>
                    </ul>
                  </div>
                </div>
              </div>
                ...
                >
                  {conversionLoading ? (
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>Convert to {type === 'panelist' ? 'Panelist' : 'Reviewer'}</span>
                    </div>
                  )}
                </Button>
              </div>
