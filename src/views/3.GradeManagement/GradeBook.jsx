...
const GradeBook = () => {
  ...
  // Filter for internal examiners only
  const internalExaminers = useMemo(() => {
    return book?.book?.examiners?.filter(examiner => examiner.type === "Internal") || [];
  }, [book?.book?.examiners]);

  // Filter for internal examiner assignments only
  const internalExaminerAssignments = useMemo(() => {
    return book?.book?.examinerAssignments?.filter(
      assignment => assignment.examiner?.type === "Internal"
    ) || [];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white py-4 rounded-lg shadow-md mx-6 mb-8">
        <div className="flex items-center justify-between px-4">
          <h2 className="text-lg font-[Inter-Medium] text-gray-900">Internal Examiners</h2>
          <div className="flex gap-2">
            <button
              className="px-3 py-1.5 text-sm font-[Inter-Medium] text-white bg-primary-600 rounded hover:bg-primary-700"
              onClick={() => navigate(`/grades/book/add-internal-examiner/${bookId}`)}
            >
        ...
        <div className="px-4 mt-4">
          <GradeBookExaminerTable 
            examiners={internalExaminers}
            bookId={bookId}
            refetchBook={refetchBook}
            book={book?.book}
            examinerAssignments={internalExaminerAssignments}
