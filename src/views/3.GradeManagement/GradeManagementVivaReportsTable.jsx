...
const getSchoolName = (student) => {
  if (student?.school?.name) return student.school.name;
  if (student?.campus?.location) return student.campus.location;
  if (student?.registrationNumber) {
    const parts = student.registrationNumber.split('/');
    if (parts.length >= 4) return parts[3]; // Branch/Campus
    if (parts.length >= 3) return parts[2]; // School code
  }
  return 'Unknown';
};
...
const GradeManagementVivaReportsTable = ({ data, proposalsData, pageSize, setPageSize, currentPage, setCurrentPage, totalCount }) => {
  ...
  const groupedBySchool = useMemo(() => groupBy(tabData, row => getSchoolName(row.student)), [tabData]);
      ...
      <div className="border-b border-gray-200 pb-4">
        <nav className="flex -mb-px gap-4 justify-end">
          <button
            ...
            className={`py-2 px-4 text-xs font-medium rounded-lg ${activeTab === "proposal-defense-completed" ? "bg-[#E8EAF6] text-[#23388F] border-2 border-[#23388F]" : "text-gray-500 hover:text-gray-700 hover:border-gray-300 border-2 border-gray-200"}`}
          >
